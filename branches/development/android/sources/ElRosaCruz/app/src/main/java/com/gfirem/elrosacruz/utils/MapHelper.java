...
import com.gfirem.elrosacruz.map.MapDirection;
...
public class MapHelper {
     ...
     */
    public Marker addMarker(double latitude, double longitude, String title, String snippet, boolean playAnimation) {
        ...
        if (playAnimation) {
            CameraPosition cameraPosition = new CameraPosition.Builder().target(new LatLng(latitude, longitude))
                    .bearing(45).tilt(90).zoom(googleMap.getCameraPosition().zoom).build();
            googleMap.animateCamera(CameraUpdateFactory.newCameraPosition(cameraPosition));
        }
    ...
    public Marker addMarker(double latitude, double longitude, String title, String snippet,
                            boolean moveCameraToLocation, boolean playAnimation) {
     ...
     */
    public Marker addMarker(double latitude, double longitude, String title, String snippet,
                            boolean moveCameraToLocation, boolean playAnimation, byte zoomLevel) {
     ...
     */
    public Marker addMarker(double latitude, double longitude, String title, String snippet,
                            boolean moveCameraToLocation, boolean playAnimation, byte zoomLevel, byte MarkerColors) {
        this.zoomLevel = zoomLevel;
        this.moveCameraToLocation = moveCameraToLocation;
        return addMarker(latitude, latitude, title, snippet, playAnimation);
    }
     ...
     */
    public void drawRoutePath(final Activity activity, final LatLng LatLng1, final LatLng LatLng2,
                              final boolean moveCameraToPosition) {
...
//		CameraUpdate cu = CameraUpdateFactory.newLatLngBounds(bounds, 25, 25, 5);
     ...
     *
     */
    public void zoomToFitLatLongs(LatLng center, LatLng[] nearest_places) {
        LatLngBounds.Builder b = new LatLngBounds.Builder();
        b.include(center);
        for (LatLng l : nearest_places) {
            b.include(l);
        }
        LatLngBounds bounds = b.build();
//		CameraUpdate cu = CameraUpdateFactory.newLatLngBounds(bounds, 25, 25, 5);
    ...
    public void zoomToFitLatLongs(ArrayList<LatLng> latlng) {
        LatLngBounds.Builder b = new LatLngBounds.Builder();
        for (LatLng l : latlng) {
            b.include(l);
        }
...
//		CameraUpdate cu = CameraUpdateFactory.newLatLngBounds(bounds, 25, 25, 5);
    ...
    public ArrayList<LatLng> extractLocations(ArrayList<Placemark> places) {
        ArrayList<LatLng> result = new ArrayList<LatLng>();
        for (Placemark item : places) {
            result.add(item.getCoordinates());
        }
        return result;
    }
     ...
     */
    public static double getDistanceBetweenLatLongs_Kilometers(LatLng latLng1, LatLng latLng2) {
        double theta = latLng1.longitude - latLng2.longitude;
        double dist = Math.sin((latLng1.latitude * Math.PI / 180.0)) * Math.sin((latLng2.latitude * Math.PI / 180.0))
                + Math.cos((latLng1.latitude * Math.PI / 180.0)) * Math.cos((latLng2.latitude * Math.PI / 180.0))
    ...
    public void getLatLongList(final LatLng startPoint, final LatLng endpoint,
        ...
        class LatLongRetriever extends AsyncTask<String, Void, Void> {
            ...
            protected Void doInBackground(String... arg0) {
                try {
                    mMapDirection = new MapDirection();
                    mDocument = mMapDirection.getXMLFromLatLong(startPoint, endpoint, MapDirection.DRIVING);
            ...
            @Override
            protected void onPostExecute(Void result) {
                mLatLongLoadedListner.LatLongs(mMapDirection.getLatLongDirectionsList(mDocument));
                super.onPostExecute(result);
            }
        }
    ...
    public void PlayRouteAnimation(final Marker[] markers, final int secondsDelay, boolean drawMarkers) {
        ...
        PolylineOptions mPolylineOptions = new PolylineOptions().width(drawRoutePath_pathWidth)
                .color(drawRoutePath_pathColor);

        for (int i = 0; i < markers.length; i++) {
            mPolylineOptions.add(markers[i].getPosition());
            if (drawMarkers)
                markers[i] = addMarker(markers[i].getPosition().latitude, markers[i].getPosition().longitude,
                        markers[i].getTitle(), markers[i].getSnippet(), false);
        }

        googleMap.addPolyline(mPolylineOptions);

        CameraPosition cameraPosition = new CameraPosition.Builder().target(markers[0].getPosition()).bearing(45)
                .tilt(90).zoom(15).build();
            ...
            public void onFinish() {
                ...
                if (++currentMarkerPosition < markers.length) {
                    ...
                    CameraPosition cameraPosition = new CameraPosition.Builder()
                            .target(markers[currentMarkerPosition].getPosition()).tilt(90)
                            .bearing(getNewBearing(markers[(currentMarkerPosition - 1)].getPosition(),
                                    markers[(currentMarkerPosition)].getPosition()))
                            .zoom(15).build();

                    markers[currentMarkerPosition].showInfoWindow();
                    markers[currentMarkerPosition - 1].hideInfoWindow();
                    googleMap.animateCamera(CameraUpdateFactory.newCameraPosition(cameraPosition), secondsDelay,
                            mCancelableCallback);
                    animateMarker(markers[currentMarkerPosition - 1], markers[currentMarkerPosition].getPosition(),
                            true);
        ...
        googleMap.animateCamera(CameraUpdateFactory.newCameraPosition(cameraPosition), mCancelableCallback);
    }
